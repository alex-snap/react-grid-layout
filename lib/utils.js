...
export type LayoutItem = LayoutItemRequired &
                         {minW?: number, minH?: number, maxW?: number, maxH?: number,
                          moved?: boolean, static?: boolean, container?: boolean,
...
export function cloneLayoutItem(layoutItem: LayoutItem): LayoutItem {
  return {
    ...
    moved: Boolean(layoutItem.moved), static: Boolean(layoutItem.static), container: Boolean(layoutItem.container),
    // These can be null
 ...
 */
export function compact(layout: Layout, verticalCompact: boolean, strategy: ?string): Layout {
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static && !l.container) {
      l = compactItem(compareWith, l, verticalCompact, strategy);
...
export function correctBounds(layout: Layout, bounds: {cols: number}): Layout {
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    ...
    if (!l.static && !l.container) collidesWith.push(l);
    else {
 ...
 * @param {MoveAwayStrategy} moveStrategy
 */
export function moveElement(layout: Layout, l: LayoutItem, x: ?number, y: ?number, isUserAction: ?boolean,
                            moveStrategy: ?string): Layout {

  const fromPlaceholder = cloneLayoutItem(l);
  ...
  for (let i = 0, len = collisions.length; i < len; i++) {
    ...
    if (collision.static) {
      layout = moveElementAwayFromCollision(layout, collision, l, isUserAction, fromPlaceholder, moveStrategy);
    } else if (!collision.container) {
      layout = moveElementAwayFromCollision(layout, l, collision, isUserAction, fromPlaceholder, moveStrategy);
    }
  }
 ...
 *                                   by the user.
 * @param  {LayoutItem} collidesWithFrom          collidesWith from clone
...
export function moveElementAwayFromCollision(layout: Layout, collidesWith: LayoutItem,
                                             ...
                                             collidesWithFrom: LayoutItem,
                                             strategy: ?string): Layout {
  switch (strategy) {
    case MoveAwayStrategy.SwapX:
      if (isUserAction) {
        ...
        fakeItem.x = (collidesWithFrom.w >= itemToMove.w && collidesWithFrom.x < itemToMove.x) ?
          collidesWithFrom.x :
          collidesWith.x + collidesWithFrom.w;
    ...
    default:
      ...
      if (isUserAction) {
        ...
        if (!getFirstCollision(layout, fakeItem)) {
          return moveElement(layout, itemToMove, undefined, fakeItem.y, false, strategy);
        }
      }
      //
